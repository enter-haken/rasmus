...
BEGIN
    CASE request->>'action'
        WHEN 'get' THEN SELECT rasmus.role_get_manager(request) INTO manager_result;
        WHEN 'add' THEN SELECT rasmus.role_add_manager(request) INTO manager_result;
        WHEN 'delete' THEN SELECT rasmus.role_delete_manager(request) INTO manager_result;
        WHEN 'update' THEN SELECT rasmus.role_update_manager(request) INTO manager_result; 
        ELSE RAISE EXCEPTION 'unknown action `%`. aborting privilege manger', request->>'action';
    END CASE;

    role_response :=  rasmus.get_entity(request->>'entity')
...
BEGIN
    ...
    IF FOUND THEN
        ...
        LOOP
            PERFORM rasmus.get_role_view((dirty_id->>'id')::UUID);
        END LOOP;
...
BEGIN
    ...
    DELETE FROM rasmus.role WHERE id = (request#>>'{data,id}')::UUID;
...
BEGIN
    ...
    PERFORM rasmus.update_privileges_for_role_if_necessary(request);

    RETURN rasmus.get_role_view((request#>>'{data,id}')::JSONB, true);
...
BEGIN
    FOR current_role_id IN SELECT id_role FROM rasmus.role_privilege WHERE id_privilege = privilege_id
    LOOP
        PERFORM rasmus.set_role_dirty(current_role_id);
...
BEGIN
    PERFORM rasmus.set_roles_dirty_for_privilege(NEW.id);
...
BEGIN
    PERFORM rasmus.set_roles_dirty_for_privilege(OLD.id);
...
BEGIN
    IF dirty_read OR EXISTS (SELECT 1 FROM rasmus.role WHERE 
            json_view IS NOT NULL AND 
            (json_view->>'is_dirty')::BOOLEAN = false AND 
            id = role_id) THEN
        SELECT json_view FROM rasmus.role WHERE id = role_id INTO role_raw;
        RAISE NOTICE 'returning undirty role %', role_id;
        RETURN role_raw;
    END IF;

    SELECT row_to_json(role) FROM
        (SELECT id, name, description, role_level FROM rasmus.role WHERE id = role_id) role INTO role_raw;
    ...
    SELECT array_to_json(array_agg(privileges)) FROM
        (SELECT p.id, 
            ...
            p.minimum_write_role_level FROM rasmus.privilege p 
        JOIN rasmus.role_privilege rp on p.id = rp.id_privilege
        JOIN rasmus.role r on r.id = rp.id_role
        WHERE r.id = role_id) privileges INTO role_privileges;

    role_raw := role_raw
        || jsonb_build_object('privileges', role_privileges)
        || rasmus.get_entity('role');

    UPDATE rasmus.role set json_view = role_raw WHERE id = role_id;
